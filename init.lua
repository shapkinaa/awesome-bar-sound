...
local function update_icon(widget, state)
   ...
   -- cr:move_to(0, 20)
   -- cr:show_text("state.volume")
   mute_text = ""
   if state.mute == true then
      mute_text = " (muted)"
   end
   widget:set_text("Volume:" .. state.volume .. "%" .. mute_text)
   -- widget:set_image(image)


   -- widget = wibox.widget.textbox()
   -- widget.set_align("right")
   -- local ctx = widget:get_state()
   -- widget.set_markup("self.widget_text")
--    widget = wibox.widget{
...
function sound.new(options)
   ...
   widgets_table = {}
   local widget = wibox.layout.fixed.horizontal() or options.widget
   -- args.widget == nil and wibox.layout.fixed.horizontal() or args.widget == false and nil or args.widget

   -- local sound_image = wibox.widget.imagebox()
   -- local widget = wibox.widget.textbox()
   -- widget.set_align("right")
   -- local sound_text = wibox.widget.textbox()
   local sound_image = wibox.widget.textbox()
   -- sound_text.font = font
   sound_image.font = font
   ...
   sound_image.set_volume = function (self, val, delta)
   ...
   sound_image.toggle_mute = function(self)
   ...
   widget_timer:connect_signal("timeout", function()
      update(state)
      update_icon(sound_image, state)
   end)
   widget_timer:start()
   sound_image:buttons(awful.util.table.join(
      awful.button({ }, 1, function()
         sound_image:toggle_mute()
      end),
   ...
   update(state)
   update_icon(sound_image, state)
   sound_image:connect_signal("mouse::enter", function() update(state) end)

   -- widget.add(sound_image)
   -- widget.add(sound_text)
   -- sound_text:set_text(state.volume)
